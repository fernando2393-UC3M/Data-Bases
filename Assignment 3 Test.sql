...
FOR X IN (
  ...
  CONTRACTS
  WHERE ((SYSDATE<ENDDATE AND SYSDATE>STARTDATE) OR (ENDDATE=NULL AND SYSDATE>STARTDATE))
) LOOP
INSERT INTO TAPS_MOVIES VALUES(X.CONTRACTID, SYSDATE - 3/24, 97, 'Titanic');
INSERT INTO TAPS_SERIES VALUES(X.CONTRACTID, SYSDATE - 3/24, 97, 'House', 1, 3);
INSERT INTO TAPS_SERIES VALUES(X.CONTRACTID, SYSDATE - 3/24, 97, 'Breaking Bad', 5, 1);
INSERT INTO TAPS_SERIES VALUES(X.CONTRACTID, SYSDATE - 3/24, 97, 'Mad Men', 1, 2);
END LOOP;
...
DELETE FROM TAPS_MOVIES WHERE(Y.CONTRACTID=CONTRACTID AND Y.PCT=97 AND Y.TITLE='Titanic');
END LOOP;
...
DELETE FROM TAPS_SERIES WHERE(Z.CONTRACTID=CONTRACTID AND Z.PCT=97 AND Z.TITLE='House' AND Z.SEASON=1 AND Z.EPISODE=3);
DELETE FROM TAPS_SERIES WHERE(Z.CONTRACTID=CONTRACTID AND Z.PCT=97 AND Z.TITLE='Breaking Bad'AND Z.SEASON=5 AND Z.EPISODE=1);
DELETE FROM TAPS_SERIES WHERE(Z.CONTRACTID=CONTRACTID AND Z.PCT=97 AND Z.TITLE='Mad Men' AND Z.SEASON=1 AND Z.EPISODE=2);
END LOOP;
