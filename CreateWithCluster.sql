...
CREATE OR REPLACE CLUSTER TITLE_MOV (movie_title VARCHAR2(100));
CREATE OR REPLACE CLUSTER TITLE_SER (title VARCHAR2(100));
CREATE CLUSTER ID_CONTRACTS (contractId VARCHAR2(10));
CREATE OR REPLACE CLUSTER ID_CLIENTS (clienttId VARCHAR2(15));
CREATE OR REPLACE INDEX IND_TITLE_MOV ON CLUSTER TITLE_MOV;
CREATE OR REPLACE INDEX IND_TITLE_SER ON CLUSTER TITLE_SER;
CREATE INDEX IND_ID_CONTRACTS ON CLUSTER ID_CONTRACTS;
CREATE OR REPLACE INDEX IND_ID_CLIENTS ON CLUSTER ID_CLIENTS;
...
CONSTRAINT FK_invcs FOREIGN KEY (clientId) REFERENCES contracts
) CLUSTER ID_CLIENTS (clientId);
